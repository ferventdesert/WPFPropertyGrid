...
using System.Windows.Controls.WpfPropertyGrid.Utils;
...
namespace System.Windows.Controls.WpfPropertyGrid
{
    ...
    public partial class PropertyGrid : Control, INotifyPropertyChanged
    {
        ...
        public static readonly DependencyProperty SelectedPropertyProperty =
            DependencyProperty.Register("SelectedProperty", typeof(object), typeof(PropertyGrid), new PropertyMetadata(null));
        ...
        {
            get { return (IList)GetValue(ListCollectionProperty); }
            set { SetValue(ListCollectionProperty, value); }
        }

        // Using a DependencyProperty as the backing store for ListCollection.  This enables animation, styling, binding, etc...
        public static readonly DependencyProperty ListCollectionProperty =
            DependencyProperty.Register("ListCollection", typeof(object), typeof(PropertyGrid), new PropertyMetadata(null));



        public ListBox ListControl
        {
            get { return (ListBox)GetValue(ListControlProperty); }
            set { SetValue(ListControlProperty, value); }
        }

        // Using a DependencyProperty as the backing store for ListControl.  This enables animation, styling, binding, etc...
        public static readonly DependencyProperty ListControlProperty =
            DependencyProperty.Register("ListControl", typeof(ListBox), typeof(PropertyGrid), new PropertyMetadata(null));
        ...
        public object SelectedObject
        {
            ...
            set
            {
                ...
                {
                    var item = new ValueWrapper(value);
                    item.ValueChanged += (s, e) =>
                        {
                            ...
                            var coll = ListCollection as IList;
                            if (coll == null)
                                return;
                         var    listIndex = ListControl.SelectedIndex;

                            if (listIndex < 0 || listIndex > coll.Count)
                                return;

                            try
                            {
                                coll[listIndex] = Convert.ChangeType(item.Item, coll[0].GetType());
                            }
                            catch (Exception)
                            {

                                item.Item = coll[listIndex];
                            }
                            
                            ListControl.ItemsSource = null;
                            ListControl.ItemsSource = ListCollection  as IEnumerable;
        ...
        */
        ...
        protected override void OnKeyDown(KeyEventArgs e)
        {
            if (e.Key == Key.Tab && e.OriginalSource is DependencyObject)//tabbing over the property editors
            {
                ...
                var t = Keyboard.Modifiers == ModifierKeys.Shift ? GetTabElement(source, -1) : GetTabElement(source, 1) ;
                 var element=t as
                UIElement;
                if (element != null)
                {
        ...
        private Object GetTabElement(DependencyObject source, int delta)
        {
